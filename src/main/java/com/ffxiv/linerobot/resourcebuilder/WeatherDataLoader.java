...
public class WeatherDataLoader {
  ...
  public static void main(String[] args) {
    ...
    try (InputStream is = WeatherDataLoader.class.getClassLoader().getResourceAsStream(fileName);
      ...
      //                    String probability = (i + 1 < items.length) ? items[i + 1] : "100"; //
      // 如果沒有概率值則默認為100
      //                    System.out.println("  Weather: " + weather + ", Probability: " +
      // probability);
      //                }
      ...
      long hoursAfterIntervalStart =
          (eorzeanEpochHours % 24) - (((eorzeanEpochHours % 24) / 8) * 8);
      ...
      System.out.println(
          "interval start earth time: " + formattedDateTime); // Output: 2021-06-29 12:13:51
      System.out.println(
          "interval start ET: "
              + ((eorzeanEpochHours - hoursAfterIntervalStart) % 24)
              + ":00"); // EorzeanTime
      System.out.println(
          "Current Earth Time:"
              + Instant.ofEpochSecond(earthEpochSeconds)
                  .atZone(zoneId)
                  .toLocalDateTime()
                  .format(formatter));
      System.out.println(
          "Current ET: " + FFXIVTimeUtil.convertEarthTimeToEorzeanTime(earthEpochSeconds));

      // use internal start time to calculate weather
      ObjectMapper mapper = new ObjectMapper();
      List<WeatherRateIndices> weatherRateIndicesList =
          JsonUtil.readJsonFileToList(
              "src/main/resources/ffxiv/resourcedata/WeatherRateIndices.json",
              WeatherRateIndices.class,
              mapper);
      List<TerritoryType> territoryTypeList =
          JsonUtil.readJsonFileToList(
              "src/main/resources/ffxiv/resourcedata/TerritoryType.json",
              TerritoryType.class,
              mapper);
      List<Weather> weatherList =
          JsonUtil.readJsonFileToList(
              "src/main/resources/ffxiv/resourcedata/Weather.json", Weather.class, mapper);
      ...
      System.out.println(
          targetTerritoryType.getPlaceName().getNameChs() + ":" + targetWeather.getNameChs());
